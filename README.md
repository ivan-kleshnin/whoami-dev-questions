...
- [x] Использовал только Low-code решения (кодинг под CMS или в одиночку)
- [ ] Использовал GIT в локальной разработке
- [ ] Использовал GIT в командной разработке
- [ ] Использовал Ansible или другие инструменты для скриптинга деплоя
- [ ] Использовал Docker и Docker-compose (или иной простой оркестратор контейнеров)
- [ ] Использовал ещё и Deploy Pipeline типа Gitlab CI/CD, Github Actions, Jenkins, Travis, etc
- [ ] Использовал K8s для продакшена, чёткая разница между подходами к разным окружениям
- [ ] Использовал Terraform, достигли реального IaC

#### Бонусы (выбрать все подходящие)

- [ ] Практикуем Trunk-based development
- [ ] Практикуем автоматический линтинг и/или компиляцию
- [ ] У нас настроены автоматические тесты (локально и в проде)
- [ ] У нас настроен (и анализируется!) авто-трекинг ошибок (Pingom & Sentry или альтернативы) (локально и в проде)
- [ ] У нас настроен (и анализируется!) авто-трекинг производительности (в проде, например New Relice)
- [ ] Наш код и dev-flow прошёл профессиональный code-review 3-й стороны
- [ ] Наш код и dev-flow прошёл профессиональный performance-review 3-й стороны
- [ ] Наш код и dev-flow прошёл профессиональный security-review 3-й стороны

### Тестирование

### Языки программирования

Под "владением" подразумевается знание синтаксиса языка, встроенных библиотек/API, основных концепций.
Для JS это асинхронность, прототипная модель, принципы ФП; для Java это многопоточность и ООП и т.д.
Под "похожими" языками имеется, в первую очередь, сходство концепций. Во вторую – сходство области применения.

#### Основное (выбрать 1 ближайший)

- [x] Владею одним языком программирования (HTML, XML, CSS не считаются, смотри бонусы)
- [ ] Владею двумя похожими языками (примеры: Python и JS, Python и Ruby, C# и Java, etc)
- [ ] Владею тремя языками, минимум две группы (примеры: Python и JS + TypeScript, Java и Python + R, Java и Erlang + Elixir)
- [ ] Владею четырьмя или более языками, минимум три группы
- [ ] Владею языками из >=4 групп (могу обосновать принципиальные различия между группами)

#### Бонусы (выбрать все подходящие)
- [ ] Знаю основные языки декларации данных (JSON, XML, YAML, TOML)
- [ ] Знаю язык со статической типизацией (1 или более)
- [ ] Знаю язык в функциональной парадигме (-//-)
- [ ] Знаю язык со статической типизацией эффектов (-//-)
- [ ] Знаю декларативные языки веб-разработки (HTML и CSS)
- [ ] Знаю язык запросов реляционных БД (SQL или близкая альтернатива)
- [ ] Знаю язык запросов графовых БД (Cypher или близкая альтернатива)
- [ ] Знаю иной специальный язык (Shading Language, Assembly, Mathematica)
...
- [ ] Регулярно проводится аудит безопасности и/или производительности
